...
/**
 * api/restore.php
 * Restores Firestore app data (classes/students/attendance/settings/users)
 * from a ZIP backup previously created by api/backup.php. Student photos
 * and the school logo live in Firebase Storage and are not affected by
 * this restore.
 *
...
for ($i = 0; $i < $zip->numFiles; $i++) {
    ...
    // Restore uploads/* entries from OLDER backups (pre-Firebase-Storage
    // migration) is intentionally not supported anymore — current photos
    // live in Firebase Storage and are untouched by this restore, which
    // only replaces Firestore data (classes/students/attendance/settings/users).
}
