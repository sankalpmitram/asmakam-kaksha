...
if (isset($_FILES['school_logo']) && $_FILES['school_logo']['error'] === UPLOAD_ERR_OK) {
    ...
    if (isset($allowed[$mime]) && $_FILES['school_logo']['size'] <= 2 * 1024 * 1024) {
        $ext = $allowed[$mime];
        $objectName = 'school-logos/' . uniqid() . '.' . $ext;
        $publicUrl = storage_upload_local_file($_FILES['school_logo']['tmp_name'], $objectName, $mime);
        if ($publicUrl) {
            $oldObject = storage_object_name_from_url($settings['school_logo'] ?? null);
            if ($oldObject) storage_delete_object($oldObject);
            $settings['school_logo'] = $publicUrl;
        }
    }
}
